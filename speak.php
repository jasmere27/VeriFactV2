...
<div style="margin-top:20px;">
    <button type="button" id="speakBtn" onclick="speakResult()" style="
        background: #2563eb;
        color: white;
        padding: 10px 16px;
        border: none;
...
function speakResult() {
    ...
    const text = document.querySelector('.result-explanation p')?.innerText || "No analysis available.";

    if (voices.length === 0) {
        alert("Voices still loading. Try again.");
        return;
    }
    ...
    <div class="trusted-sources" style="margin-top: 20px;">
        ...
        <div class="sources-list" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 12px; margin-top: 16px;">
            <?php foreach ($result['sources'] as $source): ?>
                <a href="<?php echo htmlspecialchars($source['url']); ?>" target="_blank" class="source-link" style="display: flex; align-items: center; gap: 8px; padding: 12px; background: rgba(59, 130, 246, 0.1); border: 1px solid var(--primary-blue); border-radius: 8px; color: var(--primary-blue); text-decoration: none;">
